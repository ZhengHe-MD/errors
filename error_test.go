...
func TestE(t *testing.T) {
	...
	t.Run("auto-generate op", func(t *testing.T) {
		f1 := func() {
			...
			errV := err.(*Error)
			assert.Equal(t, Op("funcForAutoGenerateOpTest"), errV.Op)
		}
		f1()
	})
}
...
func funcForAutoGenerateOpTest() error {
	return E(Invalid, "invalid arguments")
}
...
func TestNew(t *testing.T) {
	tests := map[string]struct {
		...
		"name": {"message", errors.New("message")},
	}
...
func TestNewF(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := NewF(tt.args.format, tt.args.a...); !tt.checkErr(err) {
